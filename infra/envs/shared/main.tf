...
#
#   dns         the hosted zone. Every environment resolves through it, so
#               its $0.50/month is paid once no matter how many exist.
...
#
# Apply order: shared -> prod. The first apply is manual — the role
...
module "ci_oidc" {
  ...
  # The deploy roles are per-environment and are created by
